...
void UIPass::RenderUI()
{
    ...
    for (unsigned int n = 0, vtxOffset = 0, idxOffset = 0; n < m_tDrawData[nRenderBufferIdx].CmdList.size(); n++)
    {
        ...
        for (unsigned int i = 0; i < cmdList.DrawCmd.size(); i++)
        {
            ...
            const bool invalidateShaderConstants = (((Synesthesia3D::Texture*)cmd.Texture) != m_pFontTexture || HLSL::UI_Texture2D != m_nFontTextureIdx || HLSL::UIParams->TextureSwitch != (HLSL::GPU_uint)2);
