...
rootModelDir = scriptAbsPath + "/models/pbr-test/"
modelFiles = ["sphere.obj"]
modelOutputPath = scriptAbsPath + "/../Data/models/pbr-test/"
...
pathToTextureFiles = scriptAbsPath + "/models/pbr-test/textures/"
textureOutputPath = scriptAbsPath + "/../Data/models/pbr-test/textures/"
...
{
    (dir + "/" + file, "-q -f L8") for root_, dirs_, files_ in os.walk(pathToTextureFiles) for dir in dirs_ for root, dirs, files in os.walk(pathToTextureFiles + dir) for file in files if file.lower().find("metallic.") != -1 or file.lower().find("roughness.") != -1
})
...
# Compile textures
for root_, dirs_, files_ in os.walk(pathToTextureFiles):
    for dir in dirs_:
        for root, dirs, files in os.walk(pathToTextureFiles + dir + "/"):
            for name in files:
                if name != 'Thumbs.db':
                    sourceFileIsNewer = os.path.getmtime(os.path.join(root, name)) > os.path.getmtime(os.path.realpath(__file__))
                    compiledFileExists = os.path.isfile(textureOutputPath + dir + "/" + os.path.splitext(name)[0] + ".s3dtex")
                    if sourceFileIsNewer or not compiledFileExists or forceRebuildTextures:
                        ...
                        subprocess.call(textureCompilerExe + " " + customArgs[dir + "/" + name] + " -d " + textureOutputPath + dir + "/" + " -log " + scriptAbsPath + "/Logs " + os.path.join(root, name))
                    else:
