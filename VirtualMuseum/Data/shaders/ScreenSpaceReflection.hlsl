...
void psmain(VSOut input, out float4 color : SV_TARGET)
{
    ...
    float3 envAlbedo = 0;//texCUBElod(BRDF_EnvMap, float4(mul((float3x3)BRDFParams.InvViewMat, reflectedRayDir), ComputeMipFromRoughness(roughness, BRDF::EnvMapMipCount))).rgb * BRDFParams.ReflectionFactor;
