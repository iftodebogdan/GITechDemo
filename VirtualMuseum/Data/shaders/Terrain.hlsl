...
void psmain(VSOut input, out PSOut output)
{
    ...
    output.Diffuse = tex2D(Terrain_Diffuse, texCoord);
    output.Normal = EncodeNormal(normal);
    output.Material = float4(tex2D(Terrain_MatType, texCoord).r, tex2D(Terrain_Roughness, texCoord).r, 0.f, 0.f);
    output.VertexNormal = EncodeNormal(input.Normal);
