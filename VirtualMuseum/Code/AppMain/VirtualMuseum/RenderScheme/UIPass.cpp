...
void UIPass::Update(const float fDeltaTime)
{
    ...
    // Pass input to ImGui
    if (((VirtualMuseum*)AppMain)->IsUIInFocus())
    {
    ...
    m_fAlpha += (((VirtualMuseum*)AppMain)->IsUIInFocus() ? ALPHA_PER_SECOND : -ALPHA_PER_SECOND) * pFW->GetDeltaTime();
    ...
    if (m_bShowTextureViewer)
    {
        ...
        if (ImGui::Begin("Texture viewer", &m_bShowProfiler, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize))
        {
            ...
            if (m_nTextureViewerIdx >= 0)
            {
                ...
                if (ImGui::IsItemHovered() && ((VirtualMuseum*)AppMain)->IsUIInFocus())
                {
                    ImGui::BeginTooltip();
...
void UIPass::GenerateDrawData()
{
    ...
    const bool isUIInFocus = ((VirtualMuseum*)AppMain)->IsUIInFocus();
