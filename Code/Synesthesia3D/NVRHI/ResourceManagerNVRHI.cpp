...
const unsigned int ResourceManagerNVRHI::CreateVertexFormat(const unsigned int attributeCount)
{
    VertexFormat* const vf = new VertexFormatNVRHI(attributeCount);
    return AddVertexFormat(vf);
}

const unsigned int ResourceManagerNVRHI::CreateVertexFormat(
...
{
    VertexFormat* vf = new VertexFormatNVRHI(attributeCount);
...
const unsigned int ResourceManagerNVRHI::CreateIndexBuffer(
    const unsigned int indexCount, const IndexBufferFormat indexFormat,
    const BufferUsage usage)
{
    IndexBuffer* ib = new IndexBufferNVRHI(indexCount, indexFormat, usage);
    return AddIndexBuffer(ib);
}

const unsigned int ResourceManagerNVRHI::CreateVertexBuffer(
    VertexFormat* const vertexFormat, const unsigned int vertexCount,
    IndexBuffer* const indexBuffer, const BufferUsage usage)
{
    VertexBuffer* vb = new VertexBufferNVRHI((VertexFormatNVRHI*)vertexFormat, vertexCount, (IndexBufferNVRHI*)indexBuffer, usage);
    return AddVertexBuffer(vb);
}

const unsigned int ResourceManagerNVRHI::CreateShaderProgram(const char* filePath, const ShaderProgramType programType, const char* entryPoint)
{
    ShaderProgramNVRHI* sp = new ShaderProgramNVRHI(programType);
    sp->Compile(filePath, entryPoint);
    return AddShaderProgram(sp);
}

const unsigned int ResourceManagerNVRHI::CreateTexture(
...
{
    Texture* tex = new TextureNVRHI(pixelFormat, texType, sizeX, sizeY, sizeZ, mipCount, usage);
    return AddTexture(tex);
}

const unsigned int ResourceManagerNVRHI::CreateRenderTarget(const unsigned int targetCount, PixelFormat pixelFormat,
    const unsigned int width, const unsigned int height, bool hasMipmaps, bool hasDepthStencil, PixelFormat depthStencilFormat)
{
    RenderTarget* rt = new RenderTargetNVRHI(targetCount, pixelFormat, width, height, hasMipmaps, hasDepthStencil, depthStencilFormat);
...
{
    RenderTarget* rt = new RenderTargetNVRHI(targetCount, pixelFormat, widthRatio, heightRatio, hasMipmaps, hasDepthStencil, depthStencilFormat);
    return AddRenderTarget(rt);
}

const unsigned int ResourceManagerNVRHI::CreateRenderTarget(const unsigned int targetCount,
    PixelFormat pixelFormatRT0, PixelFormat pixelFormatRT1, PixelFormat pixelFormatRT2, PixelFormat pixelFormatRT3,
    const unsigned int width, const unsigned int height, bool hasMipmaps, bool hasDepthStencil, PixelFormat depthStencilFormat)
{
    RenderTarget* rt = new RenderTargetNVRHI(targetCount,
